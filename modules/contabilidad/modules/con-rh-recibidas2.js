...
// ── Estado calculado: revisa tesoreria_mbd (nuevo) + rh_movimiento_links (viejo) ──
async function _estadoCalculado(rh) {
  // Sistema nuevo: tesoreria_mbd vinculado por numero_rh
  if (rh.numero_rh && empresa_activa?.id) {
    const { data: mbdLinks } = await _supabase
      .from('tesoreria_mbd')
      .select('id, entrega_doc')
  ...
  // Sistema viejo: rh_movimiento_links
...
async function cargarRHRecibidas() {
  ...
  // Detectar RH ya vinculados en tesoreria_mbd
  const nrosRH = filasFiltradas.map(r => r.numero_rh).filter(Boolean);
  const { data: mbdAplicados } = nrosRH.length
    ? await _supabase.from('tesoreria_mbd').select('nro_factura_doc, nro_operacion_bancaria, monto, id, entrega_doc')
        .eq('empresa_id', empresa_activa.id).in('entrega_doc', ['EMITIDO', 'OBSERVADO']).in('nro_factura_doc', nrosRH)
    : { data: [] };
  const mbdMap = new Map((mbdAplicados || []).map(r => [r.nro_factura_doc, r]));
  ...
  wrap.innerHTML = `
    <table class="tabla-nexum">
      <thead><tr>
        ...
        <th>Estado</th><th style="text-align:center">Banco</th><th style="text-align:center">Acc.</th>
      </tr></thead>
      <tbody>
        ${filasFiltradas.map(r => {
          ...
          const movLink      = mbdMap.get(r.numero_rh);
          const bancoHtml    = movLink
            ? `<div style="display:flex;flex-direction:column;align-items:center;gap:2px;cursor:pointer"
                 ...
                 onclick="_verMovBancarioLink('${escapar(r.numero_rh||'')}','RH')">
          ...
          <tr>
            ...
            <td style="text-align:center;white-space:nowrap">
              ...
              <button onclick="_bmBuscarMov('RH','${r.id}','${escapar(r.numero_rh||'')}','${escapar(nombre)}',${Number(r.monto_neto||0)},'${escapar(r.fecha_emision||'')}')" title="Vincular con movimiento bancario" style="padding:4px 8px;background:rgba(116,66,16,.15);color:#B7791F;border:1px solid rgba(116,66,16,.3);border-radius:4px;cursor:pointer;font-size:13px;margin-right:2px">📂</button>
...
// ── Vincular RH con movimiento bancario — delega a _bmBuscarMov ──
async function rhVincularManual(rhId, nombre) {
  const { data: rh } = await _supabase
    .from('rh_registros')
    .select('numero_rh, monto_neto, monto_bruto, fecha_emision')
    .eq('id', rhId).single();
  if (!rh) return;
  _bmBuscarMov('RH', rhId, rh.numero_rh || '', nombre, Number(rh.monto_neto || rh.monto_bruto || 0), rh.fecha_emision || '');
}

// ── (mantiene firma vieja por si el caché llama la versión antigua) ─
async function _rhVincularManualLegado(rhId, nombre) {
  const { data: rh } = await _supabase
    .from('rh_registros')
    .select('*, prestadores_servicios(nombre, dni)')
    .eq('id', rhId).single();
  ...
  const monto  = parseFloat(rh.monto_neto || rh.monto_bruto || 0);
  const margen = Math.max(Math.abs(monto) * 0.05, 5);

  // Buscar en tesoreria_mbd movimientos con monto similar
  const { data: movs } = await _supabase
    .from('tesoreria_mbd')
    .select('id, nro_operacion_bancaria, fecha_deposito, descripcion, proveedor_empresa_personal, monto, entrega_doc')
    .eq('empresa_id', empresa_activa.id)
    .in('entrega_doc', ['PENDIENTE', 'OBSERVADO'])
    .gte('monto', -(Math.abs(monto) + margen))
    .lte('monto', -(Math.abs(monto) - margen))
    .order('fecha_deposito', { ascending: false })
    .limit(15);

  const nroRH = rh.numero_rh || '';

  // Deduplicar por N° Op. normalizado (sin ceros iniciales) + fecha
  const _seenOp = new Set();
  const movsDedup = (movs || []).filter(m => {
    const key = (m.nro_operacion_bancaria || '').replace(/^0+/, '') + '|' + (m.fecha_deposito || '');
    if (_seenOp.has(key)) return false;
    _seenOp.add(key);
    return true;
  });
  ...
  mc.innerHTML = `
    <div class="modal-overlay" style="display:flex" onclick="if(event.target===this)this.parentElement.innerHTML=''">
      <div class="modal" style="max-width:640px;width:95%;max-height:90vh;display:flex;flex-direction:column">
        <div class="modal-header" style="flex-shrink:0">
          <h3>🔍 Vincular con banco — ${escapar(nombre)}</h3>
          <button class="modal-cerrar" onclick="this.closest('.modal-overlay').remove()">✕</button>
        </div>
        <div class="modal-body" style="flex:1;overflow-y:auto">
          <div style="padding:8px 12px;background:rgba(44,82,130,.07);border-radius:6px;margin-bottom:12px;font-size:12px">
            <strong>${escapar(nroRH)}</strong> · ${escapar(nombre)} · <strong style="color:var(--color-exito)">${formatearMoneda(monto)}</strong>
          </div>

          ${movsDedup.length ? `
            <p style="font-size:12px;font-weight:600;margin-bottom:8px;color:var(--color-texto-suave)">Movimientos bancarios con monto similar:</p>
            ${movsDedup.map(m => {
              const diff = Math.abs(Math.abs(Number(m.monto||0)) - Math.abs(monto));
              const pct  = monto > 0 ? Math.round(diff / monto * 100) : 0;
              return `
              <div style="border:1px solid var(--color-borde);border-radius:8px;padding:10px 12px;margin-bottom:8px;background:var(--color-bg-card)">
                <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;flex-wrap:wrap">
                  <div style="flex:1;min-width:0">
                    ...
                    <div style="font-size:11px;color:var(--color-texto-suave);margin-top:2px">${m.fecha_deposito||'—'} · ${escapar((m.descripcion||'').slice(0,45))}</div>
                    ${m.proveedor_empresa_personal ? `<div style="font-size:11px;margin-top:1px">${escapar(m.proveedor_empresa_personal)}</div>` : ''}
                  </div>
                  <div style="text-align:right;flex-shrink:0">
                    <div style="font-weight:700;color:var(--color-exito)">${formatearMoneda(Math.abs(Number(m.monto||0)))}</div>
                    ${diff > 0 ? `<div style="font-size:10px;color:${pct>5?'#ef4444':'#f59e0b'}">Dif: ${formatearMoneda(diff)}</div>` : '<div style="font-size:10px;color:#22c55e">✓ Exacto</div>'}
                    <button onclick="rhVincularBanco('${rhId}','${m.id}','${escapar(nroRH)}','${escapar(nombre)}')"
                      style="margin-top:4px;padding:4px 12px;background:var(--color-secundario);color:#fff;border:none;border-radius:4px;cursor:pointer;font-size:11px;font-family:var(--font)">
                      🔗 Vincular
                    </button>
                  </div>
                </div>
          ...
          ` : `<p style="font-size:12px;color:var(--color-texto-suave);margin-bottom:12px">Sin movimientos bancarios con monto similar. Usa la búsqueda manual abajo.</p>`}
          ...
          <div style="margin-top:14px;border-top:1px solid var(--color-borde);padding-top:12px">
            ...
            <div style="display:flex;gap:8px">
              ...
              <button onclick="rhBuscarNroOp('${rhId}','${escapar(nroRH)}','${escapar(nombre)}')"
                style="padding:7px 16px;background:var(--color-secundario);color:#fff;border:none;border-radius:6px;cursor:pointer;font-size:12px;font-family:var(--font);font-weight:600;white-space:nowrap">
                🔍 Buscar
              </button>
            </div>
            <div id="rhr-manual-res" style="margin-top:10px"></div>
          </div>
        </div>
...
async function rhBuscarNroOp(rhId, nroRH, nombre) {
  const q   = (document.getElementById('rhr-manual-q')?.value || '').trim();
  const res = document.getElementById('rhr-manual-res');
  if (!q || !res) return;
  ...
  if (!movs?.length) {
    res.innerHTML = '<p style="font-size:12px;color:var(--color-texto-suave);margin:4px 0">Sin resultados para ese N° de operación.</p>';
    return;
  }

  // Deduplicar por N° Op. normalizado
  const _seenQ = new Set();
  const movsQ = movs.filter(m => {
    const key = (m.nro_operacion_bancaria || '').replace(/^0+/, '') + '|' + (m.fecha_deposito || '');
    if (_seenQ.has(key)) return false;
    _seenQ.add(key);
    return true;
  });

  const estadoBadge = e => e === 'EMITIDO'
    ? '<span style="background:#2F855A;color:#fff;padding:1px 6px;border-radius:8px;font-size:10px;font-weight:700">EMITIDO</span>'
    : e === 'OBSERVADO'
    ? '<span style="background:#D69E2E;color:#fff;padding:1px 6px;border-radius:8px;font-size:10px;font-weight:700">OBSERVADO</span>'
    : '<span style="background:#C53030;color:#fff;padding:1px 6px;border-radius:8px;font-size:10px;font-weight:700">PENDIENTE</span>';

  res.innerHTML = movsQ.map(m => `
...
async function rhVincularBanco(rhId, movId, nroRH, nombre) {
  ...
  const camposLlenos = !!(
    mov?.proveedor_empresa_personal?.trim() &&
    mov?.cotizacion?.trim() &&
    mov?.oc?.trim() &&
    mov?.proyecto?.trim() &&
    mov?.concepto?.trim() &&
  ...
  document.querySelector('.modal-overlay')?.remove();
  mostrarToast(`✅ RH ${escapar(nroRH)} vinculado — Banco: ${estadoDoc}`, 'exito');
  cargarRHRecibidas();
}
