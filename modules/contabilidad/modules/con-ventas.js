...
async function _renderVentasFiltradas() {
  ...
  const filasVista = _vFiltroEstado === 'ROTO' ? covFilasV.filter(x => x.roto)
    : _vFiltroEstado
  ...
  if (resumen) resumen.innerHTML = `
    ...
    <div style="width:100%;flex-basis:100%;display:flex;align-items:center;flex-wrap:wrap;gap:8px;
      ...
      ${covFilasV.filter(x => x.roto).length ? `<span onclick="_vToggleFiltroEstado('ROTO')" style="color:#C05621;font-size:10px;font-weight:700;cursor:pointer;text-decoration:underline" title="Clic: ver solo estos comprobantes. Dentro, clic en su estado ⚠️ para ver el movimiento y por qué no cuenta.">⚠️ ${covFilasV.filter(x => x.roto).length} con un movimiento que trae su N° pero no cuenta</span>` : ''}
      ...
      <tbody>
        ${filasVista.map(({ r, cov, estado5, roto }) => {
          ...
          const onclickBanco = roto
            ? `_conDetalleVinculoRoto('VENTA','${escapar(nDoc)}','${escapar(r.nro_doc_identidad||'')}')`
            : esAplicado
          ...
          <tr>
            ...
            <td style="text-align:center;white-space:nowrap">
              ...
              <button class="btn-tabla-accion" onclick="_conciliarVentaIndividual(${conciliarArgs})" title="Conciliar con movimiento bancario" style="background:rgba(113,71,224,.1);color:#7147e0">🔗</button>
...
async function guardarVenta(id) {
  ...
  if (id) {
    ...
    const dups = await _dupBuscarCompraVenta('contabilidad_ventas', 'cliente',
      { nDoc, docIdentidad: nroId, proveedor: cliente, total: payload.total_cp, fecha });
    if (dups.length) {
      const detalle = await _dupDetalleOcurrencias(dups.map(d => ({
        ...
        _label: `${d.serie_cdp}-${d.nro_cp_inicial} · ${d.cliente||''}`,
      })));
